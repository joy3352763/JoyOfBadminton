...
final class MatchStore: ObservableObject {
    ...
    func startNextGame(servingTeam: TeamSide) {
        ...
        appendEvent(.nextGameStarted(id: UUID(), newGameIndex: nextIndex,
                                     servingTeam: servingTeam, timestamp: Date()))
    }
    ...
    func undo() {
        guard let session, let newLog = MatchEngine.undoLastEffectiveEvent(eventLog: eventLog) else {
            return
        }
        eventLog = newLog
        appendUndoMarker()
